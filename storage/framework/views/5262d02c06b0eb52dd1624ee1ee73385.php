...
<head>
    ...
    <title><?php echo e($title ?? 'Zizini.co.ke - Livestock Market'); ?></title>
...
<div x-data="{mobile:false}" class="min-h-screen">
    <header class="sticky top-0 z-40 border-b border-earth/10 bg-cream/90 backdrop-blur">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 lg:px-8">
            <a href="/" class="flex items-center gap-3">
                <img src="<?php echo e($brand['logo']); ?>" alt="Zizini.co.ke" class="h-20 w-auto object-contain md:h-24">
            </a>
    ...
    <?php if(session('status')): ?>
        <div class="mx-auto mt-4 max-w-7xl px-4"><div class="rounded-lg border border-fresh/30 bg-white p-3 text-sm font-semibold text-pasture"><?php echo e(session('status')); ?></div></div>
    <?php endif; ?>
    ...
    <footer class="mt-16 border-t border-earth/10 bg-white">
        <div class="mx-auto grid max-w-7xl gap-8 px-4 py-10 md:grid-cols-4 lg:px-8">
            <div><img src="<?php echo e($brand['logo']); ?>" class="h-14 w-auto" alt="Zizini"><p class="mt-3 text-sm text-charcoal/70">Zizini connects livestock buyers and sellers across Kenya through direct owner contact.</p></div>
...
<?php /**PATH F:\George Clients\Zizini-starter\resources\views/components/layouts/main.blade.php ENDPATH**/ ?>